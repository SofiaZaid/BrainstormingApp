...
class BrainstormingRoom extends Component {
      ...
      <div id="wrap">
        <span id="headlineRoom">Your room</span>
            <ul id="addedIdeasList">
              {this.state.items.map(item => <li key ={item.id.toString()}>{item.messageText}</li>)}
            </ul>
  ...
  getUpdateList(timeOfLastUpdate) {
    ...
    fetch('http://localhost:7071/api/newmessages/' + this.props.match.params.roomid + "/" + date)
    .then(messages => messages.json())
    .then(messages => this.setState({items: this.state.items.concat(messages)}))
    .then(() => window.setTimeout(() => this.getUpdateList(thisUpdate), milliSeconds));
  }
  
  formatTime(dateToFormat) {
    let date = dateToFormat.getFullYear()+'-'+(dateToFormat.getMonth()+1)+'-'+dateToFormat.getDate();
    let time = dateToFormat.getHours() + ":" + dateToFormat.getMinutes() + ":" + dateToFormat.getSeconds();
    return  date+' '+time;
  }
